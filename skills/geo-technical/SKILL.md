...
Generate **`clients/{client_slug}/GEO-TECHNICAL-AUDIT.md`** where `{client_slug}` is the business name derived from the target URL (strip protocol, strip www., strip TLD — keep only the business name, e.g. suncehotel.rs → suncehotel). Ensure the directory exists first: `mkdir -p clients/{client_slug}`
